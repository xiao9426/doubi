...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/xiao9426/doubi/master/bbr.sh; then
...
Download_SSR(){
	...
	env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/xiao9426/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/xiao9426/doubi/master/other/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/xiao9426/doubi/master/other/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
Install_ServerSpeeder(){
	...
	cd /root
	#借用91yun.rog的开心版锐速
	wget -N --no-check-certificate https://raw.githubusercontent.com/xiao9426/serverspeeder/master/serverspeeder.sh
...
Install_LotServer(){
	...
	#Github: https://github.com/xiao9426/serverSpeeder_Install
	wget --no-check-certificate -qO /tmp/appex.sh "https://raw.githubusercontent.com/xiao9426/serverSpeeder_Install/master/appex.sh"
...
Uninstall_LotServer(){
	...
	[[ -z ${unyn} ]] && echo && echo "已取消..." && exit 1
	if [[ ${unyn} == [Yy] ]]; then
		wget --no-check-certificate -qO /tmp/appex.sh "https://raw.githubusercontent.com/xiao9426/serverSpeeder_Install/master/appex.sh" && bash /tmp/appex.sh 'uninstall'
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/xiao9426/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf banall.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/xiao9426/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf banall.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Update_Shell(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/xiao9426/doubi/master/ssrn.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="softs"
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/xiao9426/doubi/master/ssrn.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		if [[ ${yn} == [Yy] ]]; then
			if [[ $sh_new_type == "softs" ]]; then
				wget -N --no-check-certificate https://raw.githubusercontent.com/xiao9426/doubi/master/ssrn.sh && chmod +x ssr.sh
			else
				wget -N --no-check-certificate https://raw.githubusercontent.com/xiao9426/doubi/master/ssrn.sh && chmod +x ssr.sh
			fi
